...
/* Auto-lock windows. Nothing is persisted, so closing or reloading the tab already locks the vault;
   these cover an unattended open tab, where the OS screen lock is the other line of defence. Both
   were short enough that ordinary use — a glance at another tab, a phone call — meant re-unlocking. */
...
function touch() { clearTimeout(state.lockTimer); state.lockTimer = setTimeout(() => lock("Locked after an hour without activity"), IDLE_LOCK_MS); }
function lock(reason = "Locked") {
  ...
  showEditor(false); renderEntries(); status(reason); $("name").focus();
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  for (const event of ["keydown", "pointerdown"]) document.addEventListener(event, touch); /* keydown alone locked mouse-only sessions mid-use */
  ...
  /* A hidden tab locks sooner than an active one, but not so soon that a glance elsewhere costs an unlock. */
  document.addEventListener("visibilitychange", () => { if (document.hidden) { clearTimeout(state.lockTimer); state.lockTimer = setTimeout(() => lock("Locked while the tab was in the background"), HIDDEN_LOCK_MS); } else touch(); });
